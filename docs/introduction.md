...
## Single-qubit gates
[@Li2022pulselevelnoisy]
In this documentation we are describing how a single-qubit gate that implements rotation around the x- or y-axis on the Bloch sphere can be performed.
...
In this simple example we assumed that we could ignore the higher levels of the qubit. In general leakage errors which take the qubit out of the computational subspace as well as phase errors can occur. To combat theses errors the so-called DRAG\cite{Motzoi2009,Chow2010,Gambetta2011,De2015} procedure (Derivative Reduction by Adiabatic Gate) is used. In doing so we apply an extra signal in the out-of-phase component, such that
